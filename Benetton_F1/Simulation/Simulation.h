...
public:
	...
	/// @brief Sets the engieering member
	/// @param eng used to set the engineering member
	void setEngineering(Engineering* eng);

	/// @brief Checks if a given part is in a part list
	/// @param partName is a string for the part
	/// @param ourParts is the parts vector
	/// @return true if the part is in the vector<Part*>
	bool isInOurParts(string partName, vector<Part*> ourParts);
};
